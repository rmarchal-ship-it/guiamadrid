#!/usr/bin/env python3
"""
BACKTEST v8 TIME-ADJUSTED — Momentum Breakout (1-bar delay vs 2-bar original)

Copia de backtest_experimental.py con UN SOLO CAMBIO:
  find_candidates() lee signals[idx] en vez de signals[idx-1].
  Esto reduce el retardo de 2 barras a 1 barra (dato→entrada),
  alineandose con el comportamiento real del paper_trading.

  Justificacion: signals[idx] ya tiene .shift(1) incorporado en generate_signals(),
  por lo que solo usa datos de idx-1. La senal es conocida al cierre de idx-1
  y la entrada ocurre al Open de idx. No hay look-ahead bias.

Original: backtest_experimental.py (2-bar delay, mas conservador)
---
...
def find_candidates(signals_data, active_trades, current_date, is_macro_ok,
    ...
    """
    TIME-ADJUSTED: lee signals[idx] en vez de signals[idx-1].
    signals[idx] ya tiene .shift(1) incorporado, asi que solo usa datos de idx-1.
    Esto da 1 barra de retardo (dato->entrada) en vez de 2, alineado con paper_trading.
    """
    candidates = []
    for ticker, sd in signals_data.items():
        ...
        if idx < 1:
            continue
        # TIME-ADJUSTED: signals[idx] usa datos de idx-1 (por .shift(1) en generate_signals)
        # Esto es valido: la senal se conoce al cierre de idx-1, entrada al Open de idx
...
def run_backtest(months, tickers, label, use_leverage_scaling=False,
    ...
    for current_date in all_dates:
        ...
        # Filtro macro usa Close[T-1] (ultimo cierre conocido antes de Open[T])
        if CONFIG['use_macro_filter']:
            prev_dates = [d for d in macro_bullish if d < current_date]
            is_macro_ok = macro_bullish[prev_dates[-1]] if prev_dates else True
        else:
            is_macro_ok = True
